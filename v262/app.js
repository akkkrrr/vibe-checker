/**
 * ================================================
 * VIBE CHECKER v2.6.1-polling
 * ================================================
 * Phase 2.6.1: SMART POLLING & UX IMPROVEMENTS
 * - Smart polling (2h, adaptive intervals) ✅
 * - Return to home button ✅
 * - Golden Anchor badges ✅
 * - Active session status on home ✅
 * - Updated help guide ✅
 * 
    ...
    try {
        ...
        if (!snapshot.empty) {
            ...
            if (oldData && newData.createdAt && oldData.createdAt) {
                ...
                if (newTime > oldTime) {
                    if (newData.status === "accepted") {
                        ...
                        if (!silent) notify('💕 Vibe Match! Kumppanisi hyväksyi!');
                        renderResults();
                        return true; // Found update
                    } else if (newData.status === "modified") {
                        triggerNotification('proposal_received');
                        if (!silent) notify('✏️ Kumppanisi muokkasi ehdotusta!');
                        return true; // Found update
                    }
                }
            }
            ...
            if (!oldData) {
                if (newData.status === "accepted") {
                    triggerNotification('match_found');
                    if (!silent) notify('💕 Vibe Match!');
                    renderResults();
                    return true; // Found match
                } else {
                    if (!silent) notify('✅ Kumppani on vastannut!');
                    return true; // Found response
                }
            }
            
            // NO NEW UPDATES
            if (!silent) notify('⏳ Ei uusia päivityksiä');
            return false;
        } else {
            if (!silent) notify('⏳ Kumppani ei ole vielä vastannut');
            return false;
        }
    } catch (error) {
        console.error('❌ checkForUpdates failed:', error);
        if (!silent) notify('❌ Päivityksen tarkistus epäonnistui');
        return false;
    }
}

/* ================================================
   SMART POLLING SYSTEM (Phase 2.6.1)
   ================================================
   Adaptive polling: checks frequently at first,
   then less often over time. Stops after 2h.
   ================================================ */

const POLL_SCHEDULE = [
    { duration: 30000, interval: 5000 },     // 0-30s: check every 5s
    { duration: 90000, interval: 10000 },    // 30s-2min: check every 10s
    { duration: 480000, interval: 30000 },   // 2-10min: check every 30s
    { duration: 1200000, interval: 120000 }, // 10-30min: check every 2min
    { duration: 5400000, interval: 300000 }  // 30min-2h: check every 5min
];
...
function startSmartPolling() {
    ...
    console.log('🔄 Starting smart polling (2h adaptive)...');
...
function schedulePoll() {
    ...
    // Find current schedule based on elapsed time
    let currentSchedule = POLL_SCHEDULE[0];
    let cumulativeDuration = 0;
    
    for (const schedule of POLL_SCHEDULE) {
        if (elapsed < cumulativeDuration + schedule.duration) {
            currentSchedule = schedule;
            break;
        }
        cumulativeDuration += schedule.duration;
    }
    
    // Stop if exceeded 2h
    if (elapsed > 7200000) { // 2 hours
        stopPolling();
        notify('⏸️ Automaattinen tarkistus pysäytetty (2h). Klikkaa "Tarkista nyt" päivittääksesi.');
        return;
    }
    
    nextCheckTime = Date.now() + currentSchedule.interval;
    
    pollInterval = setTimeout(async () => {
        pollCount++;
        console.log(`🔍 Auto-check #${pollCount} (interval: ${currentSchedule.interval/1000}s)`);
        
        const foundUpdate = await checkForUpdates(true); // silent mode
        
        if (foundUpdate) {
            console.log('✅ Update found! Stopping polling.');
            stopPolling();
        } else {
            updatePollingCountdown();
            schedulePoll(); // Schedule next check
        }
...
function updatePollingUI(active) {
    const statusDiv = safeGetElement('polling-status');
    if (!statusDiv) return;
...
function updatePollingCountdown() {
    const textSpan = safeGetElement('polling-text');
    const countdownSpan = safeGetElement('polling-countdown');
    
    if (!textSpan || !countdownSpan || !nextCheckTime) return;
    
    const updateCountdown = () => {
        const remaining = Math.max(0, Math.ceil((nextCheckTime - Date.now()) / 1000));
        
        if (remaining > 0) {
            countdownSpan.textContent = `(Seuraava: ${remaining}s)`;
            setTimeout(updateCountdown, 1000);
        }
    };
    
    textSpan.textContent = `🔄 Tarkistetaan automaattisesti... (#${pollCount})`;
    updateCountdown();
}
...
async function submitSelection() {
    ...
    try {
        ...
        // Kerää kortit
        safeQueryAll('.selected').forEach(el => {
        ...
        if (!time) {
            const timeSlider = safeGetElement('time-slider');
        ...
        // Kerää checkboxit
        safeQueryAll('input[type="checkbox"]:checked').forEach(c => {
        ...
        if (status === "accepted") {
            ...
            // ← Phase 2.6.1: Start smart polling
            startSmartPolling();
            
            notify("✅ Ehdotus lähetetty! Tarkistan automaattisesti kumppanin vastausta.");
        }
    } catch (e) {
...
/* ================================================
   NAVIGATION & SESSION MANAGEMENT (Phase 2.6.1)
   ================================================ */

/**
 * Return to home screen while keeping session active
 */
function returnToHome() {
    stopPolling(); // Stop auto-checking
    showScreen('welcome');
    showActiveSessionBox(); // Show active session info
    notify('🏠 Palattu etusivulle. Sessio säilyi.');
}

/**
 * Show/hide active session box on home screen
 */
function showActiveSessionBox() {
    const box = safeGetElement('active-session-box');
    const actions = safeGetElement('welcome-actions');
    const sessionIdSpan = safeGetElement('active-session-id');
    const statusSpan = safeGetElement('active-session-status');
    ...
    if (state.sessionId) {
        ...
        if (sessionIdSpan) sessionIdSpan.textContent = state.sessionId;
        
        if (statusSpan) {
            const roleText = state.userRole === 'partner_a' ? 'Partner A' : 'Partner B';
            statusSpan.textContent = `Rooli: ${roleText} | Kierros: ${state.currentRound}/${MAX_ROUNDS}`;
        }
    } else {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ← Phase 2.6: Check Updates button (manual refresh - selection screen)
    ...
    // ← Phase 2.6.1: Manual Check button (waiting screen)
    const manualCheckBtn = safeGetElement('manual-check-btn');
    if (manualCheckBtn) {
        manualCheckBtn.onclick = async () => {
            const originalHTML = manualCheckBtn.innerHTML;
            manualCheckBtn.disabled = true;
            manualCheckBtn.innerHTML = '⏳ Tarkistetaan...';
            
            if (navigator.vibrate) navigator.vibrate(10);
            
            await checkForUpdates(false); // Not silent
            
            manualCheckBtn.disabled = false;
            manualCheckBtn.innerHTML = originalHTML;
        };
    }
    
    // ← Phase 2.6.1: Return to Home button
    const returnHomeBtn = safeGetElement('return-home-btn');
    if (returnHomeBtn) {
    ...
    // ← Phase 2.6.1: Home screen - Check Updates
    const checkUpdatesHomeBtn = safeGetElement('check-updates-home-btn');
    if (checkUpdatesHomeBtn) {
        checkUpdatesHomeBtn.onclick = async () => {
            const originalHTML = checkUpdatesHomeBtn.innerHTML;
            checkUpdatesHomeBtn.disabled = true;
            checkUpdatesHomeBtn.innerHTML = '⏳ Tarkistetaan...';
            ...
            checkUpdatesHomeBtn.disabled = false;
            checkUpdatesHomeBtn.innerHTML = originalHTML;
        };
    }
    
    // ← Phase 2.6.1: Home screen - Continue Session
    const continueSessionBtn = safeGetElement('continue-session-btn');
    ...
    // ← Phase 2.6.1: Home screen - Cancel Session
    const cancelSessionHomeBtn = safeGetElement('cancel-session-home-btn');
    if (cancelSessionHomeBtn) {
        cancelSessionHomeBtn.onclick = () => {
            if (confirm('Haluatko varmasti peruuttaa session?')) {
                if (navigator.vibrate) navigator.vibrate(30);
                stopPolling();
                stopListening();
                ...
                clearAllSelections();
                window.history.pushState({}, '', window.location.pathname);
                showActiveSessionBox(); // Hide session box
                notify('✕ Sessio peruutettu');
            }
        };
    }
    ...
    // ← Phase 2.6: Service Worker DISABLED (compatibility)
    // No PWA registration to avoid browser conflicts
    console.log('ℹ️ Service Worker disabled in v2.6 for compatibility');
});
